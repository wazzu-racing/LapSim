...
class PlotTireDataPage(tkinter.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        cornering_button = tkinter.Button(self, text="Import Tire", bg="White", fg="Black",  highlightbackground="Black", font=("Ariel", 24), command= lambda: self.select_file(self))
        ...
        self.plot_button = tkinter.Button(self, text="Plot Data", bg="White", fg="Black", highlightbackground="Black", font=("Ariel", 24), state="disabled", command=lambda: self.plot_tire_data(self.dropdown.get()))
    ...
    def select_file(self, root):
        ...
        self.tire_file_path = filedialog.askopenfilename(title="Open Tire File", initialdir=file_manager.get_models_dir(), defaultextension=".pkl", filetypes=[("Pickle files", "*.pkl")])

        # If the path that the user chooses is not empty, then allow the user to plot data.
        if self.tire_file_path:
            self.plot_button.configure(state="normal")
            self.reveal_dropdown(root)
            self.tire_file_check.grid(row=2, column=2, pady=(100, 10))
        else:
            print("No file selected")

    # Function to plot tire data with functions in tire_model.py based on selected option.
    def plot_tire_data(self, option):

        # Load tire object from file
        tire = pkl.load(open(self.tire_file_path, 'rb'))
